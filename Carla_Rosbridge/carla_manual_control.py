...
class HUD(object):
    ...
    def __init__(self, role_name, width, height):
        ...
        self.data_subscriber = rospy.Subscriber("testdata", String, self.data_callback)
    ...
    def update_info_text(self):
        ...
        self._info_text = [
            ...
            'Height:  % 18.0f m' % z,
	        'Sensordata: % 20s' % self.sensordata,
            '',
            'GPS Data',
            'Satellites: % 10s' % self.gps_satellites,
            'HDOP: % 18s' % self.gps_hdop,
            'Latitude: % 16s' % self.gps_lat,
            'Longitude: % 15s' % self.gps_lon,
            'Fix (age): % 13s' % self.gps_fix_age,
            'Datetime: % 30s' % self.gps_datetime,
            'Date (age): % 12s' % self.gps_date_age,
            'Altitude: % 16s' % self.gps_altitude,
            'Course from: % 13s' % self.gps_course_from,
            'Speed: % 18s' % self.gps_speed,
            'Card from: % 12s' % self.gps_card_from,
            'Distance: % 15s' % self.gps_distance,
            'Course to: % 16s' % self.gps_course_to,
            'Card to: % 14s' % self.gps_card_to,
            'Chars: % 19s' % self.gps_chars,
            'Sentences: % 12s' % self.gps_sentences,
            'Checksum fails: % 6s' % self.gps_checksum,
            '']
        self._info_text += [
    ...
    def data_callback(self, data):
        ...
        ''' GPS module data
        gpsData[0] = Amount of satellites
        gpsData[1] = HDOP
        gpsData[2] = Latitude (deg)
        gpsData[3] = Longitude (deg)
        gpsData[4] = Fix (age)
        gpsData[5] = Datetime
        gpsData[6] = Date (age)
        gpsData[7] = Altitude (m)
        gpsData[8] = Course from GPS
        gpsData[9] = Speed
        gpsData[10] = Card from GPS
        gpsData[11] = Distance
        gpsData[12] = Course to GPS
        gpsData[13] = Card to GPS
        gpsData[14] = Chars (rx)
        gpsData[15] = Sentences (rx)
        gpsData[16] = Checksum (fail)
        '''

        gpsData = data.data.split(",")
        
        self.gps_satellites = int(gpsData[0])
        self.gps_hdop = int(gpsData[1])
        self.gps_lat = float(gpsData[2])
        self.gps_lon = float(gpsData[3])
        self.gps_fix_age = int(gpsData[4])
        self.gps_datetime = str(gpsData[5])
        self.gps_date_age = int(gpsData[6])
        self.gps_altitude = float(gpsData[7])
        self.gps_course_from = float(gpsData[8])
        self.gps_speed = float(gpsData[9])
        self.gps_card_from = str(gpsData[10])
        self.gps_distance = int(gpsData[11])
        self.gps_course_to = float(gpsData[12])
        self.gps_card_to = str(gpsData[13])
        self.gps_chars = int(gpsData[14])
        self.gps_sentences = int(gpsData[15])
        self.gps_checksum = int(gpsData[16])

        self.sensordata = gpsData # data.data
        self.update_info_text()
...
def main():
    ...
    resolution = {"width": 1120, "height": 768}
